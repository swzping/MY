...
def payment_content(root: ET.Element, app_type: int, title: str, desc: str) -> str:
    pay = root.find(".//wcpayinfo")
    if pay is None and app_type not in (2000, 2001) and not re.search(r"红包|转账|收款|付款|支付", f"{title} {desc}"):
        return ""

    def text(path: str) -> str:
        node = pay.find(path) if pay is not None else root.find(f".//{path}")
        return (node.text or "").strip() if node is not None and node.text else ""

    subtype = text("paysubtype")
    feedesc = text("feedesc")
    memo = text("pay_memo")
    status = compact_parts([
        text("receivertitle"),
        text("receiverdes"),
        text("sendertitle"),
        text("senderdes"),
    ...
    lowered = f"{app_type} {subtype} {title} {desc} {status}"
    if app_type == 2001 or "红包" in lowered:
        label = "红包"
    elif app_type == 2000 or "转账" in lowered:
        label = "转账"
    else:
        label = "支付"
    ...
    if desc and desc != title:
        details.append(desc)
    if feedesc:
        details.append(f"金额：{feedesc}")
    if memo:
        details.append(f"备注：{memo}")
    if status:
        details.append(f"状态：{status}")
    if subtype:
        details.append(f"子类型：{subtype}")
    return f"[{label}] {compact_parts(details)}".strip()
...
def format_content(local_type: int | None, content: str, resolve_media: bool, db_dir: Path | None, chat_username: str, ts: int) -> str:
    ...
    if base_type == 49:
        try:
            ...
            payment = payment_content(root, app_type, title, desc)
            if payment:
                return payment
            if app_type == 5:
